...
.catch((err)=>{console.log("something is being problem error is "+err)})
...
app.post("/create-account",async(req,res)=>{
    ...
    return res.json({error:false,user,accessToken,message:"Registration successfull"});
})
...
app.post("/login",async(req,res)=>{
    ...
    if(userInfo.email == email && userInfo.password == password)
    {
        ...
        const accessToken=jwt.sign(user,process.env.ACESS_TOKEN_SECRET,{expiresIn:"3600m"});
        return res.json({error:false,message:"Login SuccessFull",email,accessToken});
    }
    else
    {
        return res.status(400).json({error:true,message:"Invalid creadentails"});
    }
})
